  ...
  return (
    <div className="flex sm:h-screen sm:p-5">
      <div className=" flex w-full flex-col border border-t-0 bg-white sm:rounded-md sm:border-t">
        ...
        <div className="my-2 flex h-full w-full flex-col overflow-scroll p-2 dark:bg-gray-900">
          {data.data.map((job) => (
            <JobCard job={job} />
          ))}
        </div>
      </div>
      {/*<div className="flex h-full w-full flex-col overflow-y-auto p-5 last:border-b-0 "></div>*/}
    </div>
  );
};
